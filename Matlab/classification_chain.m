...
function classification_chain(Config, sD, method, sInfo, figureOfMerit, log, K)
...
for s = 1:S		
    ...
    %% Perform classification to obtain best feature set (only on features computed on consumption data, not including additional K features from classifier chain)
    sT = sConfig;
    sT.nfold = 3;
    for c = 1:C
        sT.samples{c} = training_set(1:D-K, training_truth == c);
...
%         sC.training_set = training_set;   
        sC.training_set = training_set([sTR{s}.features; [D-K+1:D]'], :);
        
        % (4) test set contains all 2^K combinations of classifier chain
        % features: [ features | D-K+k ]
        combinations = de2bi(0:2^K-1, 'left-msb');
        test_set_basis = test_set(sTR{s}.features,:);
        num_households_test = length(test_households);
        num_features_basis = length(sTR{s}.features);
        test_set_final = zeros(num_features_basis + K, num_households_test*2^K);
        ...
        for h = 1:num_households_test
            for k = 1:2^K
                % append last K digits of test samples
                idx = (h-1)*2^K+k;
                test_set_final(:,idx) = [test_set_basis(:,h); combinations(k,:)'];
