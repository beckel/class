...
function [t, p] = classify_bayes(sC)
    ...
    nb = NaiveBayes.fit(training_set', training_truth');
    [post,cpre] = posterior(nb, sC.test_set');
    
    % [t, err, posterior, logp, coeffs, distance] = classify_distance_decision_boundary(sC.test_set', training_set', training_truth', 'linear');
	t = cpre';
    p = post;
