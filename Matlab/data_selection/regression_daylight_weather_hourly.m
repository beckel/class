...
temperature.value = mean(reshape(temperature.value, 2, length(temperature.value)/2), 1)';
...
sunrise_hours = mean(reshape(sunrise_hours, 2, length(sunrise_hours)/2), 1);
sunset_hours = get_sunset_hour_from_sunrise_sunset(all_weeks, sun);
sunset_hours = mean(reshape(sunset_hours, 2, length(sunset_hours)/2), 1);
...
for dow = 1:7
    for tod = 1:24
        idx = (dow-1)*24 + tod;
        if dow == 6 || dow == 7
            weekday_weekend(2, idx:24*7:end) = 1;
        else
            weekday_weekend(1, idx:24*7:end) = 1;
        end
    end
end
...
for tod = 1:24
    ...
    idx1 = (tod-1) + 1;
    idx2 = tod;
    time_of_day(tod, idx1:24:end) = 1;
    time_of_day(tod, idx2:24:end) = 1;
end
...
for i = 1:length(ids)
    ...
    for j = 1:length(all_weeks)
        ...
        weekly_trace = mean(reshape(weekly_trace, 2, length(weekly_trace)/2), 1);
...
save('data_selection/temperature_and_daylight_variables_hourly.mat', 'temperature_and_daylight_variables');
