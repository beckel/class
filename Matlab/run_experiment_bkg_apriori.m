        ...
        if Config.apriori == 1
            ...
            for a = 1:length(apriori_class_func)
                for c = cell2mat(apriori_combinations{a})
                    fprintf('a: %d, c: %d\n', a, c);
                    data_selection_restricted(Config, str2func(class_func{c}), str2func(apriori_class_func{a}));
                ...
                for a = 1:length(apriori_class_func)
                    for c = cell2mat(apriori_combinations{a})
                        ...
                        for m = 1:length(classifiers)
                            ...
                            % analysis.
                            data_file = [Config.path_apriori, num2str(Config.weeks{1}), '/sD-', class_name, '_restrictedBy_', apriori_name, '.mat'];
                            fprintf('\nClassifying %s using %s classifier:\n\n', data_file, classifiers{m});
                            load(data_file);
                            classification(Config, sD, classifiers{m}, sInfo, figureOfMerit, log);

                            % perform classification based on apriori classes
                            data_file = [Config.path_apriori, num2str(Config.weeks{1}), '/sD-', apriori_name, '_knownAprioriWhenClassifying_', class_name, '.mat'];
                            fprintf('\nClassifying %s using %s classifier:\n\n', data_file, classifiers{m});
                            load(data_file);
