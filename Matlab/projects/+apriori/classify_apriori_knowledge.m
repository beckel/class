...
result_path = 'projects/+apriori/results/';
...
for c = 1:length(combinations)
    ...
    for p = 1:num_properties
        ...
        property = comb{num_properties,1};
        load([data_path, 'sD-', property]);
        ...
        for h = 1:num_households
            ...
            if ismember(t, labels{num_properties})
                truth(h) = 1;
            else
                truth(h) = 2;
            end
        ...
        sInfo.classes = ['APRIORI_', num2str(c), '.', num2str(p)];
        ...
        % second way: only use new features
        if p > 1
            sInfo.classes = ['APRIORI_trimmed_', num2str(c), '.', num2str(p)];
            sD.samples{1} = samples(num_features+1:num_features+p-1, idc1);
            sD.samples{2} = samples(num_features+1:num_features+p-1, idc2);
            classification(Config, sD, method, sInfo, figureOfMerit, log);
        end
    end
end
